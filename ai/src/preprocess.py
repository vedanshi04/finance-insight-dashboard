...
# def safe_parse_datetime_column(series):
#     """
#     Attempt to robustly parse datetime values with multiple known formats.
#     Falls back to pandas' internal parser if all else fails.
#     """
#     sample = series.dropna().astype(str)
#     if sample.empty:
#         return pd.Series([pd.NaT] * len(series), index=series.index)

#     # Expanded formats for edge cases
#     common_formats = [
#         "%Y-%m-%d", "%d-%m-%Y", "%m-%d-%Y",
#         "%d/%m/%Y", "%m/%d/%Y", "%Y/%m/%d",
#         "%Y-%m", "%Y/%m",  # partial year-month
#         "%b %Y", "%B %Y",  # Mar 2014, March 2014
#         "%d %b %Y", "%d %B %Y",  # 01 Mar 2014
#         "%Y-%m-%d %H:%M:%S", "%m/%d/%Y %H:%M:%S",
...
#     # Fallback to general parsing if all formats fail
#     return pd.to_datetime(series, errors="coerce")
...
#     # Remove outliers
#     df = remove_outliers_iqr(df, column_types)

#     return original_df, df, column_types
...
#             lower = Q1 - 1.5 * IQR
#             upper = Q3 + 1.5 * IQR
#             mask = df_out[col].notna() & (df_out[col] >= lower) & (df_out[col] <= upper)
#             df_out = df_out[mask]
#     return df_out